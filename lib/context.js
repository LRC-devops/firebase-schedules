...
const sessionReducer = (state, action) => {
  if (action.type === "ADD") {
    return batchSubmitHandler(action.session, action.service);
    // useEffect(() => {
...
export const SessionsProvider = ({ children }) => {
  ...
  const editHandler = (e, session, service) => {
  ...
  const addHandler = (e, session, service) => {
    dispatchSessionAction({
      type: "ADD",
      session: session,
      val: e,
  ...
  const cancelHandler = (e, service) => {
    dispatchSessionAction({ type: "CANCEL", val: e, service: service });
  };
  const deleteHandler = (session, service) => {
    dispatchSessionAction({
      type: "DELETE",
      session: session,
      service: service,
