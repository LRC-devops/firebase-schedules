...
export function addSession(e, service, serviceType) {
  ...
  if (serviceType === "sched") {
    ...
    const timestamp = new Date(s.date.value);
    const newSessObj = {
      subject: s.subject.value,
      mode: s.mode.value,
...
export async function editSession(e, id, service, serviceType) {
  let editRefArr;
  if (serviceType === "sched") {
    editRefArr = ["subject", "course", "dayTime", "host", "link", "mode"];
  } else if ((serviceType = "calendar")) {
    editRefArr = ["subject", "mode", "date"];
  }
  ...
  try {
    ...
    batch.set(
      newArr.forEach((input, i) => {
        let arrRef = editRefArr[i];
        console.log(input, editRefArr[i]);
        if (input.length > 0) {
          if (editRefArr[i] === "date") {
            const timestamp = new Date(input);
            return docRef.update(arrRef, fromDate(timestamp));
          }
