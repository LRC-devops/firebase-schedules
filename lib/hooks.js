...
export function addSession(e, service, type) {
  const s = e.target;
  console.log(e, service, type);
  ...
  if (type === "sched") {
    ...
    return toast.error(
      `${type} is not valid. Please try again, check for errors in the URL, or contact your administrator.`
    );
  }
